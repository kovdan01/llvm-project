...
public:
  ...
  bool DeterminePtrauthFailure(ExecutionContext &exe_ctx) {
    ...
    //uint64_t bad_address = 0; // m_exc_subcode;
    //uint64_t fixed_bad_address = 0xaaaaaaab07e4;//abi_sp->FixCodeAddress(bad_address);
    ...
    //   auto ptrauth_info =
    //       GetPtrauthInstructionInfo(target, arch, current_address);
    //   if (ptrauth_info && ptrauth_info->IsAuthenticated &&
    //       ptrauth_info->IsLoad) {
    //     emit_ptrauth_prologue(bad_address);
  ...
  const char *GetDescription() override {
    //if (m_description.empty()) {
      ...
      if (thread_sp) {
        ...
        if (cpu == llvm::Triple::aarch64 && DeterminePtrauthFailure(exe_ctx))
          return "PAUTH!!!";
