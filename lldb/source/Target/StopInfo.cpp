...
bool StopInfo::DeterminePtrauthFailure(ExecutionContext &exe_ctx) {
  // TODO
  ...
  if (IsBreakpoint()) {
    ...
    if (brk_ptrauth_info && brk_ptrauth_info->IsAuthenticated) {
      EmitPtrauthPrologue(strm, bad_address);
  ...
  assert(IsBadAccess() && "Handle TODO EXC_BAD_ACCESS only after this point");
  ...
  std::optional<uint64_t> bad_address = GetBadAddress();
  if (bad_address == std::nullopt)
    return false;
  uint64_t fixed_bad_address = abi_sp->FixCodeAddress(*bad_address);
