...
void StopInfoMachException::EmitPtrauthPrologue(StreamString &strm,
...
std::optional<uint64_t> StopInfoMachException::GetBadAddress() const {
...
bool StopInfoMachException::IsBadAccess() const {
  return m_value == 1; // EXC_BAD_ACCESS
}

bool StopInfoMachException::IsBreakpoint() const {
  return m_value == 6; // EXC_BREAKPOINT
}
