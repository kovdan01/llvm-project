...
CGDebugInfo::CollectPtrAuthStructAnnotations(const RecordDecl *RD) {
  ...
  llvm::Metadata *Ops[3] = {
      llvm::MDString::get(Context, StringRef("ptrauth_struct")), KeyMD, DiscMD};
  llvm::Metadata *Annotation = llvm::MDNode::get(Context, Ops);
  ...
  return DBuilder.getOrCreateArray(
      llvm::ArrayRef<llvm::Metadata *>(Annotation));
}
