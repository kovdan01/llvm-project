...
void GotSection::addAuthEntry(const Symbol &sym) {
  authEntries.push_back(
      {(numEntries - 1) * ctx.target->gotEntrySize, sym.isFunc(), sym.isUndefWeak() && !sym.isPreemptible});
}
...
void GotSection::addTlsDescAuthEntry() {
  authEntries.push_back({(numEntries - 2) * ctx.target->gotEntrySize, true, false});
  authEntries.push_back({(numEntries - 1) * ctx.target->gotEntrySize, false, false /*TODO*/});
}
